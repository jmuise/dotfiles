...
description: Use before editing files or running project commands in any repository — and whenever a tool call is blocked by require-devcontainer.sh. Holds the rule that project work happens inside the project's devcontainer, the exemption and container-detection checks, and the decision tree for getting a session containerized (which provisioning path each CLI has, and when to stop and ask the Captain first).
---
...
**No `.devcontainer/` (or k8s/Helm equivalent) in the project yet** → **confirm with the Captain before provisioning.** Retrofitting containerization onto an established repo is consequential — it adds committed files, changes how everyone opens the project, and is a judgment call about that repo's conventions. This is deliberately different from the fresh-project flow, where provisioning is automatic. Under `number-one`, escalate; in a direct session with no orchestrator above you, ask with `AskUserQuestion`. Once approved, hand containerization and the build-and-relaunch to this CLI's provisioning path. Under Claude Code that is `chief-engineer` — **Phase 3** (containerization) **+ Phase 4** (build and relaunch). Kilo and Copilot ship no provisioning role: host bootstrap necessarily runs before any container exists, so it needs the guard's by-name exemption, and only Claude Code's hook payload carries an agent identity to key one off. Under those two, stop and tell the Captain that provisioning runs under Claude Code.

**Project has `.devcontainer/` but this session simply isn't running in it** → the session needs relaunching inside it. No confirmation needed; nothing new is being decided, the baseline already exists and is just not in effect. Under Claude Code, dispatch `chief-engineer` for **Phase 4 alone** — it is written to be invoked for a single phase, so read its definition rather than restating its steps into a brief. Under Kilo or Copilot, stop and tell the Captain, per the note above.
...
`docker build`/`run`/`exec` are **not** on that allowlist. Provisioning goes through the exempt host-bootstrap role — under Claude Code, `chief-engineer`, which the hook exempts by `agent_type` because its whole job is host-side bootstrap before any container exists. The other two rosters have no such role and no such exemption, which is the intended state and not a gap to route around. Treat that exemption as the single intended path in, not as a technique to borrow — do not ask a general-purpose worker to run the docker commands on `chief-engineer`'s behalf.
