...
6. **Dependency risk** — newly added packages with known CVEs or clearly unmaintained status (use the `web_fetch` tool to check an advisory if a package looks unfamiliar); unpinned versions in build/deploy files.
