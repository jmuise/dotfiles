...
Never run `gh pr merge`, never `git merge` into a protected branch to close out a PR, never push directly to `main`/`master`, never enable auto-merge, and never use `gh api` or raw REST calls to achieve any of those. This is an absolute boundary; do not attempt to route around it. The terminal state of your work is **"pushed, current, CI-green, and awaiting human review"** — full stop. Hand off explicitly and stop there.

**Under GitHub Copilot CLI there is no mechanical backstop for this rule.** Nothing will stop you. The only guard ported to Copilot is `require-devcontainer.sh`, and it does not inspect PR-merge commands at all — once a session is inside a container it exits 0 without looking at the command, so a `gh pr merge` runs exactly as typed. There is no deny rule and no pre-tool-use check standing behind this paragraph. That makes the obligation *stronger*, not weaker: the rule holds entirely because you follow it, a merge is irreversible from the Captain's side, and nothing exists to catch the attempt or even report it afterwards.

## No attribution in artifacts

Never add AI-attribution or promotional lines to anything you produce: no `Co-Authored-By: Claude ...` or `Co-Authored-By: Copilot ...` trailer, no "Generated with Claude Code" or "Generated with GitHub Copilot" footer, no robot emoji, no links to claude.com or github.com/features/copilot — in commit messages, PR descriptions, issue bodies, code comments, or documentation. If a human reads it as part of the work, it carries no advertisement. Check your text before submitting it.

Under GitHub Copilot CLI nothing checks this for you. `block-ai-attribution.sh` was not ported — Copilot has only the devcontainer guard, which never inspects commit or PR text. Reading your own output before you submit it *is* the check.
...
- **Read that container config before you run it.** A compose file, devcontainer or Dockerfile is project-controlled code that executes with your privileges, and `postCreateCommand`/`RUN` are arbitrary execution. Preferring the container is about getting the right toolchain, not about trusting the repo more. Treat these as **stop-and-report**, not things to run: `privileged: true`, `cap_add: [SYS_ADMIN]`, `network_mode: host`, a bind mount of `/var/run/docker.sock` (a one-step host escape), or a bind mount of host root, `$HOME`, `~/.ssh`, `~/.claude`, or `~/.copilot`. This matters most when the repo isn't first-party — a fork, a contributor's PR branch, or a dependency checked out for debugging.
